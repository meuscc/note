...
export class MD extends LitElement {
  ...
  render() {
    const str = transform(this.c);
    // console.log(str);
    return html`<p id="root">${unsafeHTML(str)}</p>`;
  }
}
  ...
  return (
    text
      ...
      .replace(/\\\[((.|\s)*?)\\]/g, function ($1, $2) {
        return `<m-b m="${$2.replaceAll('"', "^_'_$")}"></m-b>`;
      })
      // 行级公式
      .replace(/\\\(((.|\s)*?)\\\)/g, function ($1, $2) {
        return `<m-i>${$2}</m-i>`;
