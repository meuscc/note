...
import { createApp, lazyHandle, useBase, useQuery } from 'h3';
...
async function handleError(error, req, res) {
  ...
  const errorObject = {
    ...
    description: !is404 ? `<pre>${stack.map((i) => `<span class="stack${i.internal ? " internal" : ""}">${i.text}</span>`).join("\n")}</pre>` : ""
  };
...
const renderMiddleware$1 = lazyHandle(() => Promise.resolve().then(function () { return render; }).then((e) => e.renderMiddleware));
app.use("/_nitro", renderMiddleware$1);
app.use(timingMiddleware);
app.use(middleware);
app.use(renderMiddleware$1);
app.stack;
...
const STATIC_ASSETS_BASE = "/_nuxt/D:/Wss/meuscc/notes/dist" + "/" + "1647598798";
const PAYLOAD_JS = "/payload.js";
const getClientManifest = cachedImport(() => import('file:///D:/Wss/meuscc/notes/.nuxt/dist/server/client.manifest.mjs'));
const getSSRApp = cachedImport(() => import('file:///D:/Wss/meuscc/notes/.nuxt/dist/server/server.mjs'));
